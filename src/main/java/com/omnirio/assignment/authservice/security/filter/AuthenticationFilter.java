package com.omnirio.assignment.authservice.security.filter;

import static com.omnirio.assignment.authservice.security.filter.SecurityConstants.EXPIRATION_TIME;
import static com.omnirio.assignment.authservice.security.filter.SecurityConstants.KEY;
...
//		Claims claims = Jwts.claims().setSubject(((org.springframework.security.core.userdetails.User) auth.getPrincipal())
//				.getUsername());
...
//	 public String createJwtSignedHMAC() throws InvalidKeySpecException, NoSuchAlgorithmException {
//
...
//	    }
		 ...
		 * InvalidKeySpecException { String rsaPrivateKey =
		 ...
		 * rsaPrivateKey.replace("-----BEGIN PRIVATE KEY-----", ""); rsaPrivateKey =
		 * rsaPrivateKey.replace("-----END PRIVATE KEY-----", "");
		 * 
		 * PKCS8EncodedKeySpec keySpec = new
		 * PKCS8EncodedKeySpec(Base64.getDecoder().decode(rsaPrivateKey)); KeyFactory kf
		 * = KeyFactory.getInstance("RSA"); PrivateKey privKey =
		 * kf.generatePrivate(keySpec); return privKey; }
		 */
